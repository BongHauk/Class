...
public class C2V_Ex01 {
	...
	public ModelAndView modelAndViewEx() { // return타입은 ModelAndView 클래스를 작성한다.
		
		//ModelAndView mv = new ModelAndView(); // ModelAndView객체를 생성한다.
		//mv.setViewName("chapter01_form/view"); // setViewName(view경로) 메서드를 사용하여 view경로를 지정한다.
		
		// 생성자의 인수로 view 경로를 지정할 수 있다.
		ModelAndView mv = new ModelAndView("chapter01_form/view");
		
		// mv.addObject(속성명, 데이터) 메서드를 사용하여 데이터를 전송한다.
		...
		return mv; // ModelAndView객체를 return한다.
