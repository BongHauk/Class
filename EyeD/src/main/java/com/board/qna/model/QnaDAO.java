...
public class QnaDAO extends DBConnPool {
    ...
    public List<QnaVO> selectListPage(Map<String, Object> map) {
        List<QnaVO> board = new ArrayList<QnaVO>();
        String query = " "
                + "SELECT * FROM ( "
    ...
    public int insertWrite(QnaVO vo) {
    ...
    public QnaVO selectView(String post_num) { //타입

        QnaVO vo = new QnaVO();  // DTO 객체 생성
        String query = "SELECT * FROM qna_board WHERE post_num=?";  // 쿼리문 템플릿 준비
        try {
...
//
    ...
    ////        String sql = "UPDATE qna_board_test SET "
...
//    }
    ...
    public int deletePost(String post_num) {
        ...
        try {
            String query = "DELETE FROM qna_board WHERE post_num=?";
            psmt = con.prepareStatement(query);
            psmt.setString(1, post_num);
            result = psmt.executeUpdate();
        } catch (Exception e) {
    ...
    public int updatePost(QnaVO vo) {
