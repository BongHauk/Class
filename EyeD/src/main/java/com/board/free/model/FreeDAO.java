...
public class FreeDAO extends DBConnPool {
    ...
    public List<FreeVO> selectListPage(Map<String, Object> map) {
    ...
    public int insertWrite(FreeVO vo) {
    ...
    public FreeVO selectView(String post_num) { //타입

        FreeVO vo = new FreeVO();  // DTO 객체 생성
        String query = "SELECT * FROM free_board WHERE post_num=?";  // 쿼리문 템플릿 준비
        try {
...
//    }
    ...
    public int deletePost(String post_num) {
        ...
        try {
            String query = "DELETE FROM free_board WHERE post_num=?";
            psmt = con.prepareStatement(query);
            psmt.setString(1, post_num);
            result = psmt.executeUpdate();
        } catch (Exception e) {
    ...
    public int updatePost(FreeVO vo) {
