...
public class TechniqueEx05 {
	...
	public static void main(String[] args) {
		...
		// [error] 싱글턴 패턴은 일반 객체 생성방법과 다르다.
		//new SingleTonTest();
		
		System.out.println(SingleTonTest.getInstance()); // 같은객체
		System.out.println(SingleTonTest.getInstance()); // 같은객체
		System.out.println(SingleTonTest.getInstance()); // 같은객체
		System.out.println();
		
		// 싱글턴클래스명.getInstance().변수명
		System.out.println(SingleTonTest.getInstance().testVar);
		
		// 싱글턴클래스명.getInstance().메서드명();
		SingleTonTest.getInstance().testMethod();
		
		System.out.println();
		System.out.println(new NotSingleTonTest()); // 다른객체
		System.out.println(new NotSingleTonTest()); // 다른객체
		System.out.println(new NotSingleTonTest()); // 다른객체
