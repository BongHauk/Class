...
/*
	...
	# execution 명시자 
	   ...
	   - execution(수식어(접근제어자)패턴 리턴타입패턴 패키지이름패턴.클래스이름패턴.메서드이름패턴(파라미터패턴))
	    		Ex)        public           void          com.spring.aop.board.controller.boardDetail(long)
...
@Component
@Aspect		// AOP Adivce클래스로 Spring Bean으로 등록한다.
public class Advice {

	/* advice에 정의된 메서드는 return을 사용할 수 없다. */
	
	/*
	@Pointcut("execution(public void com.spring.aop.*.work())") // 중복되는 타겟메서드를 기술
	public void abc() {  
		// 아무 역할을 하지 않는다.
	}
	...
	// 메서드 호출 후
	@After("execution(public * com.spring.aop.*.work())")
	//@Before("abc()")
	...
	public void aroundGetWorkingTime(ProceedingJoinPoint pjp) throws Throwable {
		...
		pjp.proceed(); // 예외처리 작업 필요
		
		// 메서드 호출 후
		long endTime = System.currentTimeMillis();
		System.out.println("메서드 호출 시간 : " + (endTime - startTime));
		System.out.println("\n=================\n");
	...
	public void afterThrowingGetError() {
		System.out.println("(afterThrowing호출)");
	}
	*/
