...
public class Upload1 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		if(files.hasMoreElements()) { // 다음 엘리먼트가 있으면
			...
			String ContentType 		= multipartRequest.getContentType(element); 		// 업로드 된 파일의 타입을 반환
			long length 			= multipartRequest.getFile(element).length(); 		// 업로드 된 파일의 크기(long 타입)를 반환
